...
def load_place_ids():
    ...
    try:
        ...
        print("❌ all_place_ids.jsonが見つかりません。")
        sys.exit(1)
...
def search_limited_routes():
    ...
    try:
        # Place IDがある物件のみを選択（最大3件）
        available_properties = []
        for prop in properties:
            if prop['name'] in place_ids['properties']:
                available_properties.append(prop)
                if len(available_properties) >= 3:
                    break
        
        # Place IDがある目的地のみを選択（最大3件）
        available_destinations = []
        for dest in destinations:
            if dest['id'] in place_ids['destinations']:
                available_destinations.append(dest)
                if len(available_destinations) >= 3:
                    break
        
        print(f"\n処理対象: {len(available_properties)}物件 × {len(available_destinations)}目的地")
        ...
        for prop_idx, prop in enumerate(available_properties, 1):
            ...
            print(f"\n{'=' * 60}")
            print(f"[{prop_idx}/{len(available_properties)}] {prop_name}")
            print(f"{'=' * 60}")
            
            # 各目的地へのルート検索
            for dest_idx, dest in enumerate(available_destinations, 1):
                dest_id = dest['id']
                dest_place_info = place_ids['destinations'][dest_id]
                
                print(f"\n  [{dest_idx}/{len(available_destinations)}] → {dest['name']}")
            ...
            # 進捗を保存
            with open('/app/output/japandatascience.com/timeline-mapping/data/properties_limited.json', 'w', encoding='utf-8') as f:
